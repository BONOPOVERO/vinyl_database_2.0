...
async function updateCenterContent(index) {
  ...
  updateContentTimeout = setTimeout(async () => {
    try {
      const vinile = filteredVinili[index];
      if (!vinile) return;

      // CONTROLLO ASINCRONO DATI MANCANTI DAL DATABASE (SQLite)
      const isMissingMetadata = (!vinile.titolo_album || !vinile.artista) && !String(vinile.id).startsWith("FALLBACK_");
      
      if (isMissingMetadata && window.sqliteWorker) {
         // 1. Gestione del Placeholder
         centerContent.innerHTML = `
          <div style="display:flex; flex-direction:column; justify-content:center; align-items:center; height:100%; text-align:center; padding: 20px;">
            <h2 style="color:#ff9ffc; margin-bottom: 15px; font-size: 1.6rem; font-weight: 800;">Caricamento vinili in corso...</h2>
            <p style="color:#9ca3af;">Recupero dati dal database...</p>
          </div>
         `;
         ...
         try {
             ...
             if (results && results.length > 0) {
                 const globalData = JSON.parse(results[0].data);
                 
                 // Mappa i campi Discogs su quelli dell'app se assenti
                 ...
                 if (cachedDataStr) {
                     ...
                     const cIdx = cachedArr.findIndex(c => String(c.id) === String(vinile.id));
                     if(cIdx !== -1) {
                         cachedArr[cIdx] = vinile;
                         localStorage.setItem('app_all_vinyls_cache', JSON.stringify(cachedArr));
                     }
                 }

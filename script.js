...
window.sanitizeHTML = function(html) {
  ...
  allElements.forEach(el => {
      for (const attr of Array.from(el.attributes)) {
          ...
          } else if (attr.value.trim().toLowerCase().startsWith('javascript:')) {
              el.removeAttribute(attr.name);
          }
      }
...
function getDiscogsToken() {
  return localStorage.getItem("app_discogs_token") || "";
}
...
window.forceRefreshDiscogsPrice = function(vinileId) {
  ...
  if (container) {
    container.safeInnerHTML = `<span style="opacity: 0.8; font-size: 0.85rem;" class="shimmer-text">🔄 Ricalcolo live in corso...</span>`;
  }
...
async function fetchDiscogsLivePrice(vinile, targetElementId = 'discogs-live-box', estimatedValue = 0, forceRefresh = false) {
  ...
  if (!vinile) {
    if (container) {
      container.safeInnerHTML = `<span style="opacity: 0.6;">⚠️ Dati insufficienti per la verifica live</span>`;
    }
    return null;
  }
  ...
  try {
    ...
    if (!barcode && !matrice) {
      if (container) {
        container.safeInnerHTML = `<span style="opacity: 0.6; font-size: 0.85rem;">⚠️ Dati insufficienti: valutazione live disabilitata</span>`;
      }
      return null;
    }
    ...
    if (container) {
      container.safeInnerHTML = `<span style="font-size: 0.76rem; opacity: 0.6;">⚠️ Impossibile sincronizzare il mercato Discogs (Offline/Rate limit)</span>`;
    }
    return null;
  }
}
...
function populateGenreSelect() {
  ...
  if (filterGenreSelect) {
    filterGenreSelect.safeInnerHTML = '<option value="">Tutti i Generi</option>' + 
...
function renderWheel() {
  ...
  if (filteredVinili.length === 0) {
    ...
    if (isGuest) {
        emptyItem.safeInnerHTML = `<div style="text-align:center; padding: 10px;">
    ...
    try {
      ...
      discogsAutoTimer = setTimeout(() => {
        if (!isDiscogsBatchSyncing) {
          ...
          const container = document.getElementById('discogs-live-box');
          if (container) container.safeInnerHTML = '<span style="opacity:0.6;">Sincronizzazione in corso...</span>';
        }
      }, 600);
    } catch (renderError) {
...
if (calcQuickValBtn) calcQuickValBtn.addEventListener('click', async () => {
  ...
  resultDiv.safeInnerHTML = '<span class="shimmer-text">🔄 Ricerca in corso su Discogs...</span>';
...
async function searchMusicBrainzOrDiscogs(query) {
  ...
  discogsResults.safeInnerHTML = '<div style="padding:10px; font-size:0.82rem; color:#ff9ffc;">🔍 Ricerca in corso...</div>';
  ...
  if (!foundAny) {
    ...
    discogsResults.safeInnerHTML = `<div class="discogs-results-list">${resultsHTML}</div>`;
...
if (forceSyncBtn) {
  forceSyncBtn.addEventListener('click', async () => {
    ...
    forceSyncBtn.safeInnerHTML = '⏳ Sincronizzazione in corso...';
    forceSyncBtn.disabled = true;
    forceSyncBtn.style.opacity = '0.7';
    
    await syncAllDiscogsPrices(true);
    
    forceSyncBtn.safeInnerHTML = origText;
  ...
  closeJukeboxModalBtn.addEventListener('click', () => {
    if (jukeboxModal) {
      ...
      if (jukeboxTimer) {
        ...
        jukeboxTimer = null;
        if (jukeboxAutoBtn) jukeboxAutoBtn.safeInnerHTML = '▶️ Autoplay Party (10s)';
      }
    }
  });
}
...
async function refreshCommunityLists() {
    ...
    listContainer.safeInnerHTML = '<p style="font-size: 0.85rem; color: #888; text-align: center;">Caricamento utenti...</p>';
    ...
    try {
      ...
      listContainer.safeInnerHTML = '<p style="font-size: 0.85rem; color: #ef4444; text-align: center;">Errore durante il caricamento: ' + e.message + '</p>';
    }
}
...
function renderCommunityLists() {
    ...
    if (profiles.length === 0) {
       listContainer.safeInnerHTML = `<p style="font-size: 0.85rem; color: #888; text-align: center;">${currentExploreTab === 'friends' ? 'Nessun utente seguito.' : 'Nessun utente trovato.'}</p>`;
    } else {
       listContainer.safeInnerHTML = '';
       profiles.forEach(p => {
...
async function openUserProfile(profile) {
  ...
  upVinylList.safeInnerHTML = '<p style="font-size: 0.85rem; color: #888; text-align: center; grid-column: 1 / -1;">Caricamento collezione remota...</p>';
  ...
  try {
      ...
      upVinylList.safeInnerHTML = '<p style="font-size: 0.85rem; color: #ef4444; text-align: center; grid-column: 1 / -1;">Errore di caricamento.</p>';
  }
}
...
function renderUserVinyls(filterState) {
    ...
    if (filtered.length === 0) {
        upVinylList.safeInnerHTML = '<p style="font-size: 0.85rem; color: #888; text-align: center; grid-column: 1 / -1;">Nessun disco in questa sezione.</p>';
        return;
    }
    ...
    upVinylList.safeInnerHTML = html;
...
function renderDatabaseResults(results, startTime) {
    ...
    if (!results || results.length === 0) {
        databaseResults.safeInnerHTML = timeText + '<div style="padding: 20px; text-align: center; color: #9ca3af;">Nessun vinile trovato.</div>';
        return;
    }
    ...
    databaseResults.safeInnerHTML = finalTimeText + uniqueResults.map(v => {
...
window.showDatabaseRecordDetails = async function(id, fromPersonal = false) {
    ...
    if (record.is_lightweight) {
        detailsContainer.safeInnerHTML = `
        ...
        try {
            ...
            console.error(e);
            detailsContainer.safeInnerHTML = '<div style="text-align: center; color: #f87171; padding: 30px;">Errore durante il caricamento dei dati.</div>';
            return;
        }
    }

...
export async function fetchDatabaseFromGitHub(username) {
  ...
  try {
    if (token) {
      ...
      if (res.ok) {
        ...
        if (localStorage.getItem('app_github_token')) {
          localStorage.removeItem('app_github_token');
          return fetchDatabaseFromGitHub(username); // retry without the bad local token
        }
      }
    }
  } catch (err) {
  ...
  try {
    const res = await fetchWithRetry(`database/${username}.json`);
...
export async function pushDatabaseToGitHub(allVinyls, username) {
  ...
  // 1. Get current SHA
  let sha = null;
  const getRes = await fetchWithRetry(apiUrl + '?t=' + Date.now(), { headers, cache: 'no-store' });
  if (getRes.ok) {
    const data = await getRes.json();
    sha = data.sha;
  } else if (getRes.status !== 404) {
    if (getRes.status === 401) { localStorage.removeItem('app_github_token'); throw new Error("Token scaduto o non valido. Il token errato è stato rimosso, riprova!"); }
  ...
  const putRes = await fetchWithRetry(apiUrl, {
    method: 'PUT',
    headers,
...
export async function fetchUserProfile(username) {
  ...
  try {
    const res = await fetchWithRetry(apiUrl, { headers, cache: 'no-store' });
    if (res.ok) {
      const data = await res.json();
      const content = decodeURIComponent(escape(atob(data.content)));
      return JSON.parse(content);
...
export async function pushUserProfile(username, profileData) {
  ...
  let sha = null;
  try {
    const getRes = await fetchWithRetry(apiUrl, { headers });
    if (getRes.ok) {
      const data = await getRes.json();
      sha = data.sha;
    }
...
export async function fetchMasterCatalogFromGitHub() {
  ...
  try {
    const res = await fetchWithRetry(`database/master_catalog.json`);
    return await res.json();
  } catch (e) {
    return [];
  }
...
export async function pushMasterCatalogToGitHub(catalogData) {
  ...
  const putRes = await fetchWithRetry(apiUrl, { method: 'PUT', headers, body: JSON.stringify(body) });
...
export async function fetchProposalsFromGitHub() {
  ...
  try {
    const res = await fetchWithRetry(`database/proposals.json`);
